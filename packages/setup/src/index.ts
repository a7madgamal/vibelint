...
function readPackageJson(): PackageJson {
  ...
  if (!existsSync(packageJsonPath)) {
    throw new Error(
      "package.json not found. Please run this command in a project directory."
    );
  }
...
function showESLintPluginInstructions(configFormat: ESLintConfigFormat): void {
  ...
  if (configFormat === "none") {
    console.log(
      kleur.yellow("No ESLint configuration found. Please set up ESLint first.")
    );
    return;
  }
  ...
  console.log(
    kleur.dim(
      "To use @vibelint/eslint-plugin-suppress-approved, add it to your ESLint configuration:\n"
    )
  );

  if (configFormat === "flat") {
    console.log(
      kleur.green("For ESLint flat config (eslint.config.mjs/js/ts):\n")
    );
...
async function main() {
  ...
  console.log(
    kleur.dim(
      `  ESLint config: ${
        eslintConfigFormat === "none"
          ? "Not found"
          : eslintConfigFormat + " config"
      }`
    )
  );
  console.log();
    ...
    const {pluginChoice} = await prompts(
      {
        ...
        name: "pluginChoice",
        message:
          "Install ESLint plugin (@vibelint/eslint-plugin-suppress-approved)?",
        initial: true,
        stdin: process.stdin,
        stdout: process.stdout,
      },
  ...
  if (selection === "lint-only" || selection === "both") {
    ...
    if (setupHook) {
      ...
      if (huskyInstalled) {
        ...
        console.log(
          kleur.yellow(
            "⚠ Husky not found. Install husky first, then add this to your pre-commit hook:"
          )
        );
        const lintCommand =
          packageManager === "pnpm"
  ...
  if (pkg.scripts?.commit) {
    console.log(
      kleur.dim(`  ${packageManager === "pnpm" ? "pnpm" : "npm"} run commit`)
    );
  }
  if (pkg.scripts?.["commit-lint"]) {
    console.log(
      kleur.dim(
        `  ${packageManager === "pnpm" ? "pnpm" : "npm"} run commit-lint`
      )
    );
  }
  ...
  if (installPlugin && eslintConfigFormat !== "none") {
    ...
    console.log(
      kleur.dim("  - Configure the ESLint plugin using the instructions above")
    );
  }
